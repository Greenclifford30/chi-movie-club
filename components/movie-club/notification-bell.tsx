...
export function NotificationBell() {
  ...
  return <div className="relative">
    <Button variant="ghost" size="icon" title="Notifications" onClick={toggleOpen}>
      ...
      {unread ? <span className="absolute right-0 top-0 grid size-4 place-items-center rounded-full bg-violet-300 text-[10px] font-bold text-slate-950">{unread > 9 ? "9+" : unread}</span> : null}
    </Button>
    {open ? <section className="absolute right-0 top-11 z-[70] w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-white/10 bg-slate-950 shadow-2xl shadow-black/40">
      ...
      <div className="max-h-96 overflow-y-auto">{loading ? <div className="flex items-center gap-2 p-4 text-sm text-slate-400"><Loader2 className="size-4 animate-spin" />Loading updates...</div> : notifications.length ? notifications.slice(0, 8).map((notification) => <Link key={`${notification.notificationId}-${notification.createdAt}`} href={notification.href} onClick={() => setOpen(false)} className={`block border-b border-white/5 px-4 py-3 transition hover:bg-white/5 ${notification.readAt ? "" : "bg-violet-300/5"}`}><p className="text-sm font-medium text-white">{notification.title}</p><p className="mt-1 text-sm text-slate-400">{notification.body}</p></Link>) : <p className="p-4 text-sm text-slate-400">No planning activity yet.</p>}</div>
      <Link href="/notifications" onClick={() => setOpen(false)} className="block px-4 py-3 text-center text-sm font-medium text-violet-300 hover:bg-white/5">View all activity</Link>
    </section> : null}
  </div>;
}
